...
function HomePage() {
	...
	const toPinGeoJSON = useCallback(
		(data: PinApiResponse[]) => ({
			type: "FeatureCollection",
			features: data.map((p) => ({
				type: "Feature",
	...
	useKeyboardShortcuts({
		...
		onCreate: async () => {
			...
			try {
				...
				console.log( "reverse geocode failure" );
			}
	...
	useEffect(() => {
		...
		if (pinId && allPins.features.length > 0) {
			const feature = allPins.features.find(
				(f) => f.properties.id === parseInt(pinId),
			);
			if (feature) {
	...
	useEffect(() => {
		...
		const fetchPins = async () => {
			try {
				...
				if (pinSort === "trending") {
					data = await pinsApi.getTrendingPins<PinApiResponse[]>(
						7,
						opts,
					);
				} else {
					...
					if (pinFilters.minDate) {
						params.set("minDate", pinFilters.minDate);
					}

					if (pinFilters.maxDate) {
						params.set("maxDate", pinFilters.maxDate);
					}

					if (pinFilters.minRating !== null) {
						params.set("minRating", String(pinFilters.minRating));
					}

					if (pinFilters.maxRating !== null) {
						params.set("maxRating", String(pinFilters.maxRating));
					}

					if (pinFilters.bookmarkStatus) {
						params.set("bookmarkStatus", pinFilters.bookmarkStatus);
					}

					const qs = params.toString();

					data = await pinsApi.getAllPins<PinApiResponse[]>(
						qs ? `?${qs}` : "",
						opts,
					);
				}
	...
	const handleMapClick = async (e: mapboxgl.MapMouseEvent) => {
		...
		if (
			...
			if (typeof clusterId === "number") {
				const source = e.target.getSource("my-data") as unknown as
					ClusterSource | undefined;
				if (!source) return;
				try {
					const zoom = await getClusterExpansionZoom(
						source,
						clusterId,
					);
					e.target.easeTo({
		...
		try {
			...
			console.log( "geocoding failure at location 3" );
		}
								...
								if (
									...
									transferPointFeatures.push({
										...
										geometry: {
											type: "Point",
											coordinates: [
												segment.arrivalLocation.lng,
												segment.arrivalLocation.lat,
											],
										},
									} as GeoJSON.Feature);
								}
							...
							features: [
								{
									...
									geometry: {
										...
										coordinates: [
											[
												destAirportCoords.lng,
												destAirportCoords.lat,
											],
			...
			<div className="main-content">
				<div className="search-container">
					...
					<button
						...
						title={
							areaSearchError ||
							"Search pins near the current map center"
						}
					>
						{isAreaSearching
							? "Searching..."
				...
				>
						...
						<DetailedPinModal
							...
							onUpdate={(updatedPoint) => {
								setAllPins((prev) => ({
									...
									features: prev.features.map((f) => {
										if (
											f.properties.id === updatedPoint.id
										) {
											return {
												...f,
												properties: {
					...
					>
						...
						{mapLayerMode === "heatmap" && (
							<Layer
								{...(HEATMAP_LAYER_STYLE as HeatmapLayerSpecification)}
							/>
						)}
